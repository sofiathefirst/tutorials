...
bool setsrvPosition()
{
	//srvposition.z = cloud.points[index].z;
	//srvposition.x = cloud.points[index].x;
	//srvposition.y = -cloud.points[index].y;
        
	if(isnan(srvposition.x) || isnan(srvposition.y) || isnan(srvposition.z))
		return false;
	//if(srvposition.z>2 ||srvposition.z < 1.2 ) return false;
	//ROS_INFO("DEPTH : %f,%f,%f",srvposition.x,srvposition.y,srvposition.z);
	
	Matrix<float,4,1> Pr;
	Matrix<float,4,1> Pc;
	Pc << 	srvposition.x*1000,
	  	srvposition.y*1000,
	  	srvposition.z*1000,
		    1;
	...
	Pr = mat_kinect *Pc;
/*
	srvposition.x = Pr(0,0)/1000;
	srvposition.y = Pr(1,0)/1000;
	srvposition.z = Pr(2,0)/1000;*/

	srvposition.x = Pr(0,0)/1000;
	srvposition.y = Pr(1,0)/1000;
	srvposition.z = Pr(2,0)/1000;

	ROS_ERROR("rotation:%f,%f,%f",srvposition.x,srvposition.y,srvposition.z);
...
Mat depthROI = Mat::zeros(imgRGB8u.size(), CV_8U);
...
drawContours(depthROI, contours, maxConnectDomainIn, Scalar(255), CV_FILLED);
areas[maxConnectDomainIn] = -100;
...
//imgDepth16u.ptr<ushort>((unsigned int)pt.Y)[(unsigned int)pt.X];
		...
		srvposition.z = Pt3D.Z/1000.;
		srvposition.x = Pt3D.X/1000.;
		srvposition.y = Pt3D.Y/1000.;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		imshow("Depth2", depthROI);
//imwrite("Depth2.bmp", imgDepthShow);
		key = waitKey(20);
