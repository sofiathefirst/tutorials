...
{
//pcl::PointCloud<pcl::PointXYZ> cloud; 
  sensor_msgs::Image image_;ROS_INFO("3");
...
bool setsrvPose()
{
	if(cloud.points.size()<1) return false;
	...
	if(isnan(srvpose.position.x) || isnan(srvpose.position.y) || isnan(srvpose.position.z))
		return false;
	if(srvpose.position.z>1.7 ||srvpose.position.z < 1.0 ) return false;
	...
	if (srvpose.position.z >-0.1 && srvpose.position.z < 0.15)
    		srvsend = true;
...
bool getsrvPose(object_detect::poseDepth::Request &req,object_detect::poseDepth::Response &res)
{
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)								  //callback function defination
  {
	...
	cv_ptr->image.copyTo(DistortedImg);	

   // cv::FileStorage fs("desktop_ref.xml", cv::FileStorage::WRITE);
   // fs<<"desktop_ref"<<cv_ptr->image;

    cv::namedWindow("desktopimg");
    cv::imshow("desktopimg", cv_ptr->image);
    ...
    fs["desktop_ref"]>>ref;
Mat pFrame;
    cv::Mat tt = ref - cv_ptr->image ;
    tt.convertTo(pFrame, CV_32FC1);
ROS_INFO("2");
...
ROS_INFO("2");
	...
	for (; itcon != contours.end(); ++itcon)
	{
		...
		Point2f vertices[4];
		rRect.points(vertices);ROS_INFO("2");
		for (int i = 0; i < 4; ++i)
		...
		gwidth = (int)rRect.size.width<(int)rRect.size.height? (int)rRect.size.width:(int)rRect.size.height;ROS_INFO("2");
		//srvsend = false;
		if(rRect.size.width> rRect.size.height)
		{
		    Point2f pttA[2];ROS_INFO("2");
			pttA[0].x = (vertices[0].x + vertices[1].x) / 2;
			pttA[0].y = (vertices[0].y + vertices[1].y) / 2;
			pttA[1].x = (vertices[2].x + vertices[3].x) / 2;
			pttA[1].y = (vertices[2].y + vertices[3].y) / 2;ROS_INFO("2");
			Point2f pttCent;
			pttCent.x = (pttA[0].x + pttA[1].x) / 2;
			pttCent.y = (pttA[0].y + pttA[1].y) / 2;ROS_INFO("2");
  			//ROS_ERROR("pttCent%d,%d",(int)pttCent.x,(int)pttCent.y);
			center2d.x =(int) pttCent.x;
			center2d.y =(int) pttCent.y;ROS_INFO("2");
			srvsend = setsrvPose();ROS_INFO("2");

			ROS_INFO("CENTER2D , x,y%d,%d",(int)center2d.x,(int)center2d.y);
			line(result, pttA[0], pttA[1], CV_RGB(255, 0, 0), 2, 8, 0);
			circle(result, pttCent, 2, Scalar(0), 3);ROS_INFO("2");
	...
	ROS_INFO("2");
		...
		else
		{
			...
			Point2f pttCen;ROS_INFO("2");
			pttCen.x = (ptt[0].x + ptt[1].x) / 2;ROS_INFO("2");
			pttCen.y = (ptt[0].y + ptt[1].y) / 2;ROS_INFO("2");
			center2d.x =(int) pttCen.x;
			center2d.y =(int) pttCen.y;ROS_INFO("2");
			srvsend = setsrvPose();
ROS_INFO("2");
...
ROS_INFO("2");
		...
		if(srvsend)
		{
			//object_detect::poseDepth::Response d;
			srvpose.orientation = gq;
			break;
			//d.pose = srvpose;
			//d.width = gwidth;
			//srvdatas.push_back(d);
		}
