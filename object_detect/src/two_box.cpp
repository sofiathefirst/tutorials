...
#include"object_detect/CornerMatch.h"
//#include"kmeans.h"
using namespace cv;
using namespace std;
...
int cnt = 0;
cv::Mat R = Mat(3, 3, CV_64F, rotation);
cv::Mat T = Mat(3, 1, CV_64F, translation);
cv::Mat D1 = Mat(5, 1, CV_64F, _D1);
cv::Mat D2 = Mat(5, 1, CV_64F, _D2);
cv::Mat M1 = Mat(3, 3, CV_64F, _M1);
cv::Mat M2 = Mat(3, 3, CV_64F, _M2);
...
bool test()
{
	...
	if (pw.size()>0)
	{
		...
		//std::cout << "[ " << pcenter3.x << ", " << pcenter3.y << ", " << pcenter3.z << " ]" <<endl;//ros::Time::now()-time.data<< endl;
		//time.data=ros::Time::now();
	}
	
        finish = clock();
	double time = ((double)finish - startT) / 1000;//µ¥Î»ÎªÃë
