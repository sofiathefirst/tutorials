...
bool findLeftCenter(vector<Point> contour,geometry_msgs::Point &leftcorner , geometry_msgs::Point &center  )
{
  ...
  {	if(!receive) return;
	...
	imgroi = imgroi(rOI);
	//cv::FileStorage fs("/home/lzc/moveit/cabnet_refROI380.xml", cv::FileStorage::WRITE);
    ...
    cv::Mat ref;
    cv::FileStorage fs("/home/lzc/moveit/cabnet_refROI380.xml", cv::FileStorage::READ); 
 ...
 void imageCbc(const sensor_msgs::ImageConstPtr& msg)								  //callback function defination
  {
    ...
    catch (cv_bridge::Exception& e)
    {
      ROS_ERROR("cv_bridge exception: %s", e.what());
      return;
    } imwrite("/home/lzc/moveit/cabimg.jpg",cv_ptr->image);
	colorimg = cv_ptr -> image;
	colorimg = colorimg(rOI);
imwrite("/home/lzc/moveit/cabimgroi.jpg",colorimg);
}
...
0,0,0,1;
  ...
  image_sub_ = it_.subscribe("/camera/ir/image_raw", 1, imageCb);      
